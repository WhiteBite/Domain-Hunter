...
## AIエージェントと自動化用（CLI + MCP）

Domain Hunter は Node CLI と MCP サーバーを同梱し、ブラウザーアプリと同じコアロジックを公開します。どちらも stdout に JSON を出力し、進行状況とログは stderr のみに出力されます。
...
終了コード：`0` 成功、`1` ランタイムエラー、`2` 使用法エラー。JSON 構造は `cli/contract.ts` を参照してください。
